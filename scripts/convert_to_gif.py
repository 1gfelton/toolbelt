...
if len(sys.argv) < 2:
    print("Usage: python convert_to_gif.py path/to/your/video/file.mp4")
    sys.exit()
...
output_dir = os.path.join(base_output_dir, video_basename, 'gif')
os.makedirs(output_dir, exist_ok=True)

# Update output path for frames
out_name = f"{video_basename}.gif"
output_path = os.path.join(output_dir, out_name)

print(f"Extracting first frame of: {video_file}")
print(f"Output directory: {output_dir}")

cmd = ['ffmpeg', '-i', video_file, output_path]
...
try:
    ...
    if result.returncode == 0:
        print("Successfully converted video to gif")
    else:
